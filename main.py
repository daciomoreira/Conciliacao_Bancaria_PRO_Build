...
from src.data_loader import ler_ofx, carregar_relatorio_dataframe, converter_dataframe
from src.reconciliation import Conciliador
from src.styling import colorir_linhas, colorir_linhas_agregado
...
def main():
        ...
        if ofx_file and rel_file:
            with st.spinner("Processando..."):
                try:
                    ...
                    # Armazenar o resultado completo antes de aplicar filtros
                    st.session_state.df_resultado_completo = df_resultado.copy()
                    ...
                    for row in st.session_state.aggregator_rows:
                        if row["values"][6] == "Não conciliado":
                            ...
                            row["values"][6] = f"Não conciliado (Dif: R$ {diferenca:.2f})"
    ...
    if st.session_state.df_resultado is not None:
        # Sempre mostrar o Resumo por Dias usando o df_resultado_completo
        if hasattr(st.session_state, 'df_resultado_completo'):
            # Criar uma nova instância do Conciliador com os dados completos
            df_completo = st.session_state.df_resultado_completo
            
            # Inicializar o Conciliador com os dados completos
        ...
        if formato_exportacao == "Excel (.xlsx)":
            ...
            # Adicionar opções para o usuário escolher o que incluir no PDF
            st.markdown("#### Selecione as seções para incluir no PDF:")
            col1, col2, col3 = st.columns(3)
            with col1:
                incluir_detalhes = st.checkbox("Detalhes da Conciliação", value=True)
            with col2:
                incluir_agregado = st.checkbox("Movimentações Agregadas", value=True)
            with col3:
                incluir_grafico = st.checkbox("Dados do Gráfico Diário", value=True)
            ...
            # Adicionar informação da conta selecionada
            if 'conta_filtro' in locals() and conta_filtro:
                account_style = styles['Heading3']
                account_style.alignment = 1  # Centralizado
                elements.append(Paragraph(f"Conta: {conta_filtro}", account_style))
            ...
            def create_table(df, title, status_col=None):
                ...
                # Ajustar largura para tabela de agregações diárias para ficar consistente com as outras
                if title == "Movimentações Agregadas por Dia":
                    # Garantir que a tabela de agregações use a largura total disponível
            ...
            # Adicionar tabelas ao PDF com base nas opções selecionadas
            if incluir_agregado:
            ...
            if incluir_grafico and 'df_diario' in st.session_state and not st.session_state.df_diario.empty:
    ...
    **Guia Rápido:**
    ...
    6. Exporte os resultados para Excel e/ou PDF quando necessário
    """)
