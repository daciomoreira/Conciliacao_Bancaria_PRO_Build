...
class Conciliador:
    ...
    def _processar_conciliacoes_com_feedback(self, progress_bar, status_text):
        ...
        for i, ofx_item in enumerate(nao_conciliadas_ofx):
            ...
            if i % 10 == 0:  # A cada 10 transações
                ...
                # Formatação correta: ponto como separador de milhar, vírgula como decimal
                valor_str = f"R$ {abs(ofx_item['valor']):,.2f}".replace(',', 'X').replace('.', ',').replace('X', '.')
    ...
    def _gerar_dataframe(self):
        ...
        for item in self.resultado:
            ...
            linha = {
                ...
                # Formatação correta: ponto como separador de milhar, vírgula como decimal
                "Extrato Valor": f"R$ {ofx['valor']:,.2f}".replace(',', 'X').replace('.', ',').replace('X', '.') if ofx else "",
                "Extrato Descrição": ofx["descricao"] if ofx else "",
                "Relatório Data": rel["data"].strftime('%d/%m/%Y') if (rel and pd.notnull(rel["data"])) else "",
                # Formatação correta: ponto como separador de milhar, vírgula como decimal
                "Relatório Valor": f"R$ {rel['valor']:,.2f}".replace(',', 'X').replace('.', ',').replace('X', '.') if rel else "",
    ...
    def _mostrar_resumo_dias(self):
        ...
        # Calcular total de transações e conciliações
        total_transacoes = len(self.trans_ofx)
        transacoes_conciliadas = len([r for r in self.resultado if r["status"].startswith("Conciliado")])
        ...
        with col2:
            st.metric(
                label="Conciliações",
                value=f"{transacoes_conciliadas}/{total_transacoes}",
                delta=f"{(transacoes_conciliadas/total_transacoes*100):.2f}%" if total_transacoes > 0 else "0%"
            )
        ...
        with col3:
            ...
            st.metric(
                label="Total Extrato", 
                value=f"R$ {total_extrato:,.2f}".replace(',', 'X').replace('.', ',').replace('X', '.')
            )
        ...
        with col4:
            ...
            st.metric(
                ...
                value=f"R$ {total_relatorio:,.2f}".replace(',', 'X').replace('.', ',').replace('X', '.'),
                delta=f"Diferença: R$ {diferenca_total:,.2f}".replace(',', 'X').replace('.', ',').replace('X', '.'),
    ...
    def agrupar_por_dia(self, rows):
        ...
        for trans in self.trans_ofx:
            ...
            if "descricao" in trans and any(termo in trans["descricao"].lower() for termo in ["saldo", "sdo", "sld", "saldo inicial", "saldo final"]):
                continue
                
            # Verificar se a data existe e é válida
            if not trans.get('data'):
                continue
        ...
        aggregated_rows = []
        for date_str, data in sorted(rows_by_date.items(), key=lambda x: datetime.strptime(x[0], "%d/%m/%Y")):
            extrato_total = data['extrato_total']
            relatorio_total = data['relatorio_total']
            
            # Formatar os valores para exibição com separador de milhar (padrão brasileiro)
            extrato_valor_fmt = f"R$ {extrato_total:,.2f}".replace(',', 'X').replace('.', ',').replace('X', '.')
            relatorio_valor_fmt = f"R$ {relatorio_total:,.2f}".replace(',', 'X').replace('.', ',').replace('X', '.')
            ...
            # Calcular a diferença para exibição no status quando não conciliado
            if status == "Não conciliado":
                diferenca = extrato_total - relatorio_total
                status = f"Não conciliado (Dif: R$ {diferenca:,.2f})".replace(',', 'X').replace('.', ',').replace('X', '.')
            
            aggregated_row = {
                "values": [
                    date_str,
                    extrato_valor_fmt,
                    " ",  # Espaço em branco (coluna separadora fina)
                    date_str,
                    relatorio_valor_fmt,
                    " ",  # Espaço em branco (coluna separadora fina)
                    status
