...
def ler_ofx(arquivo_ofx):
    ...
    try:
        ...
        for transacao in ofx.account.statement.transactions:
            ...
            # Filtrar entradas de saldo - pular transações que contêm palavras-chave de saldo
            palavras_saldo = ["SALDO ANTERIOR", "SALDO TOTAL", "SALDO DISPONIVEL", "SALDO DISPONÍVEL", 
                             "SALDO DIA", "SALDO FINAL", "SALDO EM", "SALDO CONTA"]
            
            # Verificar se a descrição contém alguma das palavras-chave de saldo
            if any(palavra in descricao.upper() for palavra in palavras_saldo):
                continue
